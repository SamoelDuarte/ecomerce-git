        ...
        <div class="card-body">
          <div class="row">
            <div class="col-lg-8 offset-lg-2">
              ...
              <form id="pageForm" action="{{ route('user.additional_section.update', $section->id) }}" method="POST">
                @csrf
                <div class="row">
                  ...
                  <div class="col-lg-6">
                    <div class="form-group p-0">
                      ...
                      <select name="possition" class="form-control select2">
                        ...
                        <option @selected($section->possition == 'hero_section') value="hero_section">
                          {{ __('After Hero Section') }}</option>

                        @foreach ($sections as $sectionName)
                          @php
                            //label for the section
                            if ($sectionName == 'tab_section') {
                                $label = __('Tabs Section');
                            } elseif ($sectionName == 'cta_section_status') {
                                $label = __('Call To Action Section');
                            } elseif ($sectionName == 'categoryProduct_section') {
                                $label = __('Category Product Section');
                            } else {
                                $label = __(ucwords(str_replace('_', ' ', $sectionName)));
                            }
                          @endphp
                          <option value="{{ $sectionName }}" @selected($section->possition == $sectionName)>
                            {{ __('After') }}
                            {{ $label }}
                          </option>
                        @endforeach
