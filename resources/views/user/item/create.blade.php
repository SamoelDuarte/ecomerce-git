    ...
    <script>
        document.getElementById('codeExcelInput').addEventListener('change', function(e) {
            ...
            reader.onload = function(e) {
                ...
                const dataRows = rows.slice(1); // remove cabeçalho

                const variations = {};
                let total = 0;

                dataRows.forEach(row => {
                    const variation = row[0] !== undefined && row[0] !== null ? row[0].toString()
                        .trim() : '';
            ...
            if (file.name.endsWith('.csv')) {
                reader.readAsBinaryString(file); // CSV: texto puro
            } else {
                reader.readAsArrayBuffer(file); // XLSX
            }
        });
    </script>
    <script>
