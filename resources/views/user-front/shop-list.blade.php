        ...
        <div class="col-12">
            <div class="product-default product-list-card product-column radius-md mb-30">
                ...
                <div class="product-price">
                    @php
                        $isDigital = $item->type === 'digital'; // ou a lógica que você usa pra saber se é digital
                        $hasZeroPrice = $item->current_price == 0.0;
                        $codes = $item->digitalCodes; // relação definida
                        $codePrices = $codes->pluck('price')->filter();
                        $minCodePrice = $codePrices->min();
                        $maxCodePrice = $codePrices->max();
                    @endphp

                    @if ($isDigital && $hasZeroPrice && $codePrices->count())
                    ...
                    @elseif ($flash_status == true)
                        ...
                        <span class="old-price">
                            {{ $item->flash_amount }}% {{ $keywords['OFF'] ?? __('OFF') }}
                        </span>
                    @else
                        <span class="new-price">
                            {{ symbolPrice($user_currency->symbol_position, $user_currency->symbol, currency_converter($item->current_price)) }}
                ...
                <div class="btn-icon-group">
                    @php
                        $hasCodes = \App\Models\User\DigitalProductCode::where(
                            'user_item_id',
                            $item->item_id,
                        )->exists();
                    @endphp
                    ...
                    @if ($shop_settings->catalog_mode != 1)
                        @if ($item->type === 'digital' && $hasCodes)
                            <a href="{{ route('front.user.productDetails', [getParam(), 'slug' => $item->product_slug]) }}"
                                class="btn btn-md btn-primary radius-sm mb-20" data-bs-toggle="tooltip" data-bs-placement="top"
