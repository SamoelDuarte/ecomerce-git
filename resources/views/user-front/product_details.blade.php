                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb justify-content-start">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">
                            {{ $keywords['Product_Details'] ?? __('Product Details') }} </li>
                    </ol>
                </nav>
            </div>
        </div>
                                    ...
                                    @foreach ($product->item->sliders as $slide)
                                        <div class="product-single-single-item">
                                            <figure class="radius-sm lazy-container ratio ratio-1-1">
                                                <a
                                                    src="{{ asset('assets/front/img/user/items/slider-images/' . $slide->image) }}">
                                                    <img class="lazyload"
                            ...
                            <div class="product-price">
                                ...
                                @else
                                    @if ($flash_status == true)
                                        <div class="new-price-area d-flex color-primary">
                                            <span class="new-price-currency">
                                                {{ $user_currency->symbol_position == 'left' ? $user_currency->symbol : '' }}
                                            </span>
                                            <span class="new-price" id="details_new-price"
                                                data-base_price="{{ currency_converter($product_current_price) }}">
                                    ...
                                    @else
                                        <div class="new-price-area d-flex color-primary">
                                            {{ $user_currency->symbol_position == 'left' ? $user_currency->symbol : '' }}
                                            <span class="new-price" id="details_new-price"
                                                data-base_price="{{ currency_converter($product->item->current_price) }}">
                                                {{ currency_converter($product->item->current_price) }}
                                            </span>
                                            {{ $user_currency->symbol_position == 'right' ? $user_currency->symbol : '' }}
                                        </div>
                                        ...
                                        @foreach ($product_variations as $product_variation)
                                            ...
                                            @foreach ($product_variation_contents as $product_variation_content)
                                                ...
                                                <li class="list-item" data-variant_name="{{ @$variant_content->name }}">
                                                    <h4 class="list-item-title color-primary mb-1">
                                                        {{ @$variant_content->name }}:</h4>
                                                        ...
                                                        @foreach ($variant_content_options as $variant_content_option)
                                                            @php
                                                                ...
                                                                $sup_option_content = make_input_name(
                                                                    @$variant_option_contents->option_content
                                                                        ->option_name,
                                                                );

                                                                $id_name = make_input_name(
                                                                    @$variant_option_contents->option_content
                                                                        ->option_name,
                                                                );
                                                                $main_id =
                                                                    'detail_' . $sup_option_content . '_' . $id_name;
                                                            @endphp
                                                            <li>
                                ...
                                @if ($shop_settings->catalog_mode != 1)
                                    <div class="d-flex flex-wrap align-items-center gap-10 mt-20 mb-20">
                                        <div class="quantity-input d-flex item_quantity_details">
                                            ...
                                            <input class="quantity_field" type="number" name="cart-amount"
                                                value="1" min="1">
                                            <div class="quantity-up quantity-btn plus"
                                                data-item_id="{{ $product->id }}">
                                        ...
                                        <div class="btn-icon-group btn-inline">
                                            ...
                                            @php
                                                ...
                                                $checkWishList = $customer_id
                                                    ? checkWishList($product->item_id, $customer_id)
                                                    : false;
                                            @endphp
                                            <a href="#"
                                ...
                                @if (Auth::guard('customer')->user())
                                    @if (App\Models\User\UserOrderItem::where('customer_id', Auth::guard('customer')->user()->id)->where('item_id', $product->item->id)->exists())
                                        <div class="comment-form">
                                            ...
                                            @enderror
                                            <h4 class="mb-10">
                                                {{ $keywords['Add Your review'] ?? __('Add Your review') }}
                                            </h4>
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="section-title title-inline mb-10">
                            <h2 class="title fs-1 mb-20">{{ $keywords['Related Products'] ?? __('Related Products') }}
                            </h2>
                            @if (count($related_product) > 0)
                                <a href="{{ route('front.user.shop', [getParam(), 'category' => $category_slug]) }}"
                                    class="btn btn-md btn-primary radius-sm mb-20">{{ $keywords['More_Items'] ?? __('More Items') }}
                                </a>
                            @endif
                        </div>
                    </div>
                                ...
                                @foreach ($related_product as $item)
                                    <div class="product-default product-center radius-xl">
                                        ...
                                        <div class="product-details">
                                            <a
                                                href="{{ route('front.user.shop', ['category' => $item->category->slug, getParam()]) }}">
                                            ...
                                            @if ($shop_settings->item_rating_system == 1)
                                                <div class="d-flex justify-content-center align-items-center">
                                                    <div class="product-ratings rate text-xsm">
                                                        <div class="rating"
                                                            style="width:{{ $item->item->rating * 20 }}%"></div>
                                                    </div>
                                                    <span
                                                        class="ratings-total">({{ reviewCount($item->item_id) }})</span>
                                                </div>
                                            @endif
                                            ...
                                            <div class="product-price">
                                                @if ($flash_status == true)
                                                    ...
                                                    <span class="old-price ms-1 line_through">
                                                        {{ symbolPrice($user_currency->symbol_position, $user_currency->symbol, currency_converter($item->item->current_price)) }}
                                                    </span>
                                                ...
                                                @else
                                                    ...
                                                    <span class="old-price line_through">
                                                        {{ symbolPrice($user_currency->symbol_position, $user_currency->symbol, currency_converter($item->item->previous_price)) }}
                                                    </span>
                                                @endif
                                            </div>
                                        </div>
                                        ...
                                        <div class="btn-icon-group btn-inline">
                                            ...
                                            @php
                                                $customer_id = Auth::guard('customer')->check()
                                                    ...
                                                    : null;
                                                $checkWishList = $customer_id
                                                    ? checkWishList($item->item_id, $customer_id)
                                                    : false;
                                            @endphp
                                            <a href="#"
                                                class="btn btn-icon radius-sm {{ $checkWishList ? 'remove-wish active' : 'add-to-wish' }}"
                                            ...
                                            <a onclick="addToCompare('{{ route('front.user.add.compare', ['id' => $item->item_id, getParam()]) }}')"
                                                class="btn btn-icon radius-sm" data-bs-toggle="tooltip"
